...
function countRoutes(locations: number[], start: number, finish: number, fuel: number): number {
  const dp: number[][] = new Array(locations.length);
  ...
  function dfs(pos: number, fuel: number): number {
    if (dp[pos][fuel] !== -1) return dp[pos][fuel];
    dp[pos][fuel] = 0;
    if (Math.abs(locations[pos] - locations[finish]) > fuel) return 0;
    for (let i = 0; i < locations.length; i++) {
      if (i !== pos) {
        const cost = Math.abs(locations[i] - locations[pos]);
        if (cost <= fuel) {
          dp[pos][fuel] += dfs(i, fuel - cost);
          dp[pos][fuel] %= MOD;
        }
      }
    }
