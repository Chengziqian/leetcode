...
function numsGame(nums: number[]): number[] {
  ...
  ans[0] = 0;
  const lower = new PriorityQueue<number>((a, b) => a > b);
  const upper = new PriorityQueue<number>((a, b) => a < b);
  lower.add(Math.min(list[0], list[1]));
  upper.add(Math.max(list[0], list[1]));
  let lowerSum = lower.front();
  let upperSum = upper.front();
  ans[1] = Math.abs(upperSum - lowerSum);
  for (let i = 2; i < list.length; i++) {
    if (list[i] <= lower.front()) {
      lower.add(list[i]);
      lowerSum += list[i];
    } else {
      upper.add(list[i]);
    ...
    if (lower.size() === upper.size() + 2) {
      const u = lower.remove();
      lowerSum -= u;
      upper.add(u);
      upperSum += u;
    }
    if (lower.size() + 1 === upper.size()) {
      const u = upper.remove();
      upperSum -= u;
      lower.add(u);
      lowerSum += u;
    }
    ans[i] = (i & 1 ? upperSum - lowerSum : upperSum - lowerSum + lower.front()) % MOD;
  }
  return ans;
};
