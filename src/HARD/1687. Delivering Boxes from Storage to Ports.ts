...
function boxDelivering(boxes: number[][], portsCount: number, maxBoxes: number, maxWeight: number): number {
  const dp: number[] = new Array(boxes.length + 1).fill(0);
  const weightSum: number[] = new Array(boxes.length + 1).fill(0);
  const diffSum: number[] = new Array(boxes.length).fill(0);
  for (let i = 1; i <= boxes.length; i++) {
    const port = boxes[i - 1][0];
    const weight = boxes[i - 1][1];
    weightSum[i] = weightSum[i - 1] + weight;
    if (i > 1) {
      const prePort = boxes[i - 2][0];
      diffSum[i] = diffSum[i - 1] + (prePort !== port ? 1 : 0);
    }
  }
  const queue: number[] = [0];
  for (let i = 1; i <= boxes.length; i++) {
    while (queue.length && (i - queue[0] > maxBoxes || weightSum[i] - weightSum[queue[0]] > maxWeight)) {
      queue.shift();
    }
    dp[i] = dp[queue[0]] + diffSum[i] - diffSum[queue[0] + 1] + 2;
    const min = dp[i] - diffSum[i + 1];
    while (queue.length && min < dp[queue[queue.length - 1]] - diffSum[queue[queue.length - 1] + 1]) queue.pop();
    queue.push(i);
  }
