...
function findMaxValueOfEquation(points: number[][], k: number): number {
  ...
  for (let i = 1; i < points.length; i++) {
    while (queue.length && points[i][0] - queue[0][1] > k) queue.shift();
    if (queue.length) ans = Math.max(ans, points[i][0] + points[i][1] + queue[0][0]);
    while (queue.length && points[i][1] - points[i][0] >= queue[queue.length - 1][0]) queue.pop();
