...
function stoneGameV(stoneValue: number[]): number {
  ...
  function dfs(start: number, end: number) {
    ...
    const sliceArray = stoneValue.slice(start, end + 1);
    const sliceArraySum = sliceArray.reduce((p, c) => p + c, 0);
    let sumLeft = 0;
    for (let i = start; i < end; i++) {
      sumLeft += stoneValue[i];
      const sumRight = sliceArraySum - sumLeft;
      if (sumLeft < sumRight) {
        dp[start][end] = Math.max(dp[start][end], dfs(start, i) + sumLeft);
      } else if (sumLeft > sumRight) {
        dp[start][end] = Math.max(dp[start][end], dfs(i + 1, end) + sumRight);
      } else {
        dp[start][end] = Math.max(dp[start][end], Math.max(dfs(start, i) + sumLeft, dfs(i + 1, end) + sumRight));
      }
    }
    return dp[start][end];
  }
