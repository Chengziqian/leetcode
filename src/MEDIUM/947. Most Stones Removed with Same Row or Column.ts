...
function removeStones(stones: number[][]): number {
  ...
  const colMap: Map<number, number[]> = new Map<number, number[]>();
  const rowMap: Map<number, number[]> = new Map<number, number[]>();
  ...
  for (let i = 0; i < stones.length; i++) {
    const [row, col] = stones[i];
    const rowList = rowMap.get(row);
    const colList = colMap.get(col);
    for (let k = 0; k < rowList.length; k++) {
      uf.union(i, rowList[k]);
    }
    for (let k = 0; k < colList.length; k++) {
      uf.union(i, colList[k]);
    }
  }
