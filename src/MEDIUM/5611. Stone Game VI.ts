...
function stoneGameVI(aliceValues: number[], bobValues: number[]): number {
  const values: Pair<number, number>[] = new Array(aliceValues.length);
  ...
  values.sort((a, b) => b.first - a.first);
  let aliceScore = 0;
  let bobScore = 0;
  for (let i = 0; i < values.length; i++) {
    if (i % 2 === 0) aliceScore += aliceValues[values[i].second];
    else bobScore += bobValues[values[i].second];
  }
