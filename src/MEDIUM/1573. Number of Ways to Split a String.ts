...
function numWays(s: string): number {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    dfs(i + 1, getCount(substr), 1);
  }
  return ans;
  function dfs(index: number, count: number, depth: number) {
    if (index > s.length || depth > 3) return;
    if (index === s.length && depth === 3) {
      ans++;
      return;
    } 
    for (let i = index; i < s.length; i++) {
      const substr = s.slice(index, i + 1);
      if (count === getCount(substr)) {
        dfs(i + 1, count, depth + 1);
      }
    }
  }
