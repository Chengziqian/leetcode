...
// MEDIUM https://leetcode-cn.com/problems/nearest-exit-from-entrance-in-maze/
  ...
  int nearestExit(vector<vector<char>>& maze, vector<int>& entrance) {
    ...
    while (!q.empty()) {
      ...
      for (int i = 0; i < size; ++i) {
        P cur = q.front();
        if (ans != 0 && (cur.first == 0 || cur.first == row - 1 || cur.second == 0 || cur.second == col - 1)) return ans;
        q.pop();
        for (int k = 0; k < 4; ++k) {
          int nextI = cur.first + direction[k][0];
          int nextJ = cur.second + direction[k][1];
          if (nextI >= row || nextI < 0 || nextJ >= col || nextJ < 0) continue;
          if (vis[nextI][nextJ] || maze[nextI][nextJ] == '+') continue;
