...
function numSplits(s: string): number {
  const charCountRight = new Array(26).fill(0);
  const charCountLeft = new Array(26).fill(0);
  let rightUnionCount = 0, leftUnionCount = 0;
  for (let i = 0; i < s.length; i++) {
    const index = getCharCode(s[i]);
    if (charCountRight[index] === 0) rightUnionCount++;
    charCountRight[index]++;
  }
  if (rightUnionCount === 1) return s.length - 1;
  let ans = 0;
  for (let i = 0; i < s.length - 1; i++) {
    const index = getCharCode(s[i]);
    charCountRight[index]--;
    if (charCountRight[index] === 0) rightUnionCount--;
    if (charCountLeft[index] === 0) leftUnionCount++;
    charCountLeft[index]++;
    if (leftUnionCount === rightUnionCount) ans++;
  }
  return ans;
