...
public:
    bool isMatch(string s, string p) {
      ...
      if (cur.size()) capture.push_back(cur);
      return dfs(s, 0, capture, 0);
    }
    bool dfs(string& s, int stringIndex, vector<string>& capture, int captureIndex) {
      if (stringIndex == s.size() && captureIndex == capture.size()) return true;
      if (captureIndex >= capture.size()) return false;
      string captureString = capture[captureIndex];
      if (captureString[captureString.size() - 1] == '*') {
        if (dfs(s, stringIndex, capture, captureIndex + 1)) return true;        
        int ci = 0;
        for (int si = stringIndex; si < s.size(); ++si) {
          if (captureString[ci] != '.' && s[si] != captureString[ci]) return false;
          ci++;
          if (ci % (captureString.size() - 1) == 0) {
            if (dfs(s, si + 1, capture, captureIndex + 1)) return true;
            else ci = 0;
          }
        }
      } else {
